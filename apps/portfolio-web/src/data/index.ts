...
export const projects: Project[] = [
  {
    ...
    caseStudy: {
      problem: [
        "디자인 시스템이 없어 화면 단위로 디자인이 생성됐고, 스타일 코드와 개발 코드가 분리되지 않아 재사용이 어려웠습니다.",
        "어떤 컴포넌트가 존재하는지 파악할 방법이 없었습니다.",
        "디자이너 작업이 개발에 반영되지 않거나, 개발·수정 내용이 디자이너에게 공유되지 않아 디자인↔개발 간극이 컸습니다.",
        "솔루션마다 테마 수가 4개·2개·1개로 제각각이라(예: EXD는 mint/blue × basic/dark 4테마) UI가 파편화돼 있었습니다.",
      ],
      process: [
        "컴포넌트 44개·아이콘 376개를 표준화하고 Storybook 문서 40개로 가시화했습니다.",
        "색·타이포·스페이싱을 토큰 컨트랙트(contract)로 두고 솔루션별 테마를 주입 — 솔루션×테마를 자유롭게 전환하도록 설계해 파편화된 테마 체계를 하나로 흡수했습니다.",
        "AI(Claude)를 도구로 활용해 초기 시스템을 구축하고, 리뷰를 받아 개선사항을 반영했습니다.",
      ],
      outcome: [
        "UI가 전면 정리되고, 어떤 컴포넌트가 있는지·무엇이 부족하고 개선해야 하는지 한눈에 파악 가능해졌습니다.",
        "컴포넌트를 미리 만들어 두고 각자 개발하다 배포 일정이 정해지면 완성본을 즉시 통합·확인 — 제품 적용 속도가 빨라졌습니다.",
        "스타일과 기능 개발을 병행할 수 있게 됐습니다.",
        "실제 4개 제품(ExD·XDR·SOAR·AirWorks)에 우선 적용하기로 결정됐고, 현재 ONE UI 파일럿이 진행 중이며 내년 컨퍼런스에서 신제품으로 공개될 예정입니다.",
      ],
      media: [
        {
          kind: "after",
  ...
  },
  {
    ...
    caseStudy: {
      ...
      problem: [
        "보안 관제 분석가는 대량의 로그를 빠르게 인지·판단해야 하지만, 복잡한 데이터가 화면에 과밀하게 노출되면 원하는 로그를 찾고 해석하기까지 인지 부하가 커집니다.",
        "검색 조건 설정부터 결과 확인까지의 흐름을 더 매끄럽게 만들 필요가 있었습니다.",
      ],
      process: [
        "디자인 시스템 컴포넌트 20여 개를 소비해 화면을 구성 — 일관된 UI로 퍼블리싱하고 부족한 컴포넌트는 직접 개발했습니다.",
        "화면설계서(ExD_logsearch) 기반으로 검색 전 위젯 / 검색 후 결과 영역의 정보구조와 화면 흐름을 설계했습니다.",
        "AI 쿼리바(AND/OR/IN·필드 검색 파싱), 스트리밍 검색(실행·일시정지·완료 + 경과 타이머), 드래그 선택 히스토그램, 로그 상세 확장, 키워드 하이라이트, 컬럼 자동 맞춤, 검색 이력·템플릿을 구현했습니다.",
      ],
      outcome: [
        "정보구조·화면 흐름·시각적 일관성을 확보해 로그를 인지·조회하는 흐름을 개선했습니다.",
        "디자인 시스템이 실제 제품 화면에서 동작함을 입증하는 소비 예시가 됐습니다.",
      ],
      media: [
        {
          kind: "after",
